...
# Fonction pour charger les données d'anime pour la section découverte
                        ...
                        if 0 <= episode_num - 1 < len(eps):
                            ...
                            for lang in langs:
                                try:
                                    ...
                                    # Utiliser ep.languages.availables qui retourne dict[Lang, list[Players]]
                                    if lang in ep.languages.availables:
                                        # availables[lang] retourne une list[Players]
                                        players_list = ep.languages.availables[lang]

                                        # Pour chaque objet Players dans la liste
                                        for players in players_list:
                                            if players and isinstance(players, list):
                                                # Trier les players par ordre de préférence
                                                # 1. Sibnet (très fiable)
                                                # 2. Vidmoly
                                                # 3. SendVid
                                                # 4. OneUpload
                                                # 5. MixDrop 
                                                ...
                                                # Classifier les players (Sibnet en priorité)
                                                sibnet_urls = [url for url in players if "sibnet" in url.lower()]
                                                vidmoly_urls = [url for url in players if "vidmoly" in url.lower()]
                                                ...
                                                other_urls = [url for url in players if "vidmoly" not in url.lower() and 
                                                             "sendvid.com" not in url and 
                                                             "oneupload.to" not in url and 
                                                             "mixdrop.co" not in url and 
                                                             "dood" not in url and
                                                             "sibnet" not in url.lower()]

                                                # Ajouter les URLs dans l'ordre de préférence (Sibnet en premier)
                                                if sibnet_urls:
                                                    lang_urls.extend(sibnet_urls)
                                                if vidmoly_urls:
...
def keep_alive():
    ...
    while True:
        try:
            ...
            scraper.get("https://test-l7ug.onrender.com")
